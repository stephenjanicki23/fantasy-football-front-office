...
**Decision:** the application was first built in a `fantasy-football/` subdirectory of
that repository, leaving the `rellax` library untouched, and has since been extracted into
this dedicated repository with its history intact. It now sits at the repository root, so
no build tool needs a root-directory setting.
...
Tests: `npm test`. Typecheck: `npm run typecheck`. See `README.md`.
